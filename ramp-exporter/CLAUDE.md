...
- Key field locations (confirmed from live data):
  ...
  - Date — deliberately **different** per journal, per accrual-accounting guidance from the CFO: the Purchase (expense debit + AP liability credit) is dated when the bill was received/entered into Ramp, while the Payment (clearing AP + cash out) is dated when funds actually left the bank. `_expand_bill()`: `created_at` → `draft_bill_created_at` → `issued_at` → `accounting_date` (defensive fallbacks only — `created_at` is present on every bill). `_expand_payment()`: `payment.payment_date` → `payment.effective_date` → `paid_at`. An earlier version collapsed both onto the payment date, which correctly fixed the Payments side but wrongly moved the Purchase off its accrual date too — reverted per CFO clarification.
  - Date Due / Discount Date (Purchases Journal only): `row["due_date"]`, sourced from `bill.get("due_at")` in `_expand_bill()` and consumed by `sage_formatter.build_csv()` (falls back to `row["date"]` if absent, so card transactions — which never set `due_date` — are unaffected). Purely AP aging/scheduling metadata; doesn't drive which period the GL entry posts to, so it can legitimately land in a later period than `Date`.
